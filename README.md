...
要素の種類はクリーンアーキテクチャの層に対応させてあり、描いた図を Markdown にしてコーディングエージェントに渡し、構成を一緒に詰める。
...
`main` に push すると、GitHub Actions（`.github/workflows/deploy.yml`）がビルドして GitHub Pages に公開する。リポジトリの Settings → Pages の Source は「GitHub Actions」にしておく。
...
- **Export for AI** で、UseCase ごとの流れ・層との対応・確認したい点（依存の向き・つながっていない要素）・Mermaid の図を Markdown にして、コピーか保存をする
...
| `src/graph/agentMarkdown.ts` | コーディングエージェント向けの Markdown の書き出し |
...
| `src/components/` | Canvas・サイドバー・編集パネル・書き出しの画面 |
